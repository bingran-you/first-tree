/**
 * Read/write `~/.first-tree/github-scan/inbox.json` with atomic rename and cross-process
 * advisory locking.
 *
 * SINGLE-WRITER RULE (spec doc 2 §1.3):
 * -------------------------------------
 * Only a poller loop — the Rust `github-scan-runner` fetcher, the TS daemon
...
function serializeEntry(entry: InboxEntry): Record<string, unknown> {
  return {
    ...
    labels: entry.labels,
    github_scan_status: entry.github_scan_status,
  };
}
