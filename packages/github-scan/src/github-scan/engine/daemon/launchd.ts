...
/** `com.first-tree.github-scan.runner.<login>.<profile>` — matches Rust `launchd_label`. */
export function launchdLabel(login: string, profile: string): string {
  return `com.first-tree.github-scan.runner.${sanitizeFilename(login)}.${sanitizeFilename(profile)}`;
}
 ...
 * interactive shell that started GitHub Scan.
 */
...
/**
 * Build the plist XML for a github-scan-runner launchd job. Byte-shape match
