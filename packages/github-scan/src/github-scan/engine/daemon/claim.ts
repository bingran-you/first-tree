 ...
 * TypeScript github-scan daemon.
 *
 * Port of `lock.rs`.
 *
 * SINGLE-WRITER RULE (spec doc 2 §1.3):
 * -------------------------------------
 * This module is **read-only** with respect to `~/.first-tree/github-scan/inbox.json`.
  ...
  for (let attempt = 1; attempt <= maxTries; attempt += 1) {
    try {
      ...
      if (existing && !isLockStale(existing)) {
        throw new Error(
          `github-scan daemon is already running for ${existing.login} on ${existing.host} (pid ${existing.pid}, profile "${existing.profile}")`,
        );
      }
