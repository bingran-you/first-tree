...
export interface TaskCandidate {
  ...
  repo: string;
  /** Operator-routing override; defaults to `repo` before dispatch. */
...
export interface DispatcherOptions {
   ...
   */
  claimTtlSec?: number;
  /** Optional bound tree repo passed down from the umbrella CLI. */
  treeRepo?: string;
  /** Optional snapshot writer. When present, task snapshots are hydrated before agent execution. */
  snapshotWriter?: Pick<GhClient, "writeTaskSnapshot">;
}
...
export class Dispatcher {
  ...
  private async runTask(
    ...
    const request = makeAgentRequest({
      ...
      treeRepo: this.options.treeRepo,
    });
