/**
 * TS port of `github-scan-status-manager`.
 *
 ...
 *
 * Usage (identical to `bash bin/github-scan-status-manager`):
 *   get <notification-id>
...
// Re-export of the `GitHubScanStatus` enum for CLI-arg validation.
const GITHUB_SCAN_STATUSES = ["new", "wip", "human", "done"] as const;
...
export interface StatusManagerDeps {
  ...
  paths?: ReturnType<typeof resolveGitHubScanPaths>;
...
function printHelp(io: StatusManagerIO): void {
  io.stdout("Usage: github-scan-status-manager <command> [args]");
  io.stdout("");
  io.stdout("Commands:");
...
function parseIsoUtc(s: string): number | null {
  // Accept the exact format `bin/github-scan-status-manager` writes
...
async function cmdGet(
  ...
  io.stdout(entry?.github_scan_status ?? "new");
  return 0;
}
...
async function cmdSet(
  ...
  const { io } = deps;
  const id = args[0];
  const status = args[1] as GitHubScanStatus | undefined;
  if (!id || !status) {
  ...
  const oldStatus: GitHubScanStatus = entry.github_scan_status;

  const gh = deps.gh ?? new GhClient();
  const repo = entry.repo;
  const num = entry.number;

  // Step 1 (spec doc 3 §3.2): always remove every github-scan:* label first.
  for (const lbl of ALL_GITHUB_SCAN_LABELS) {
    gh.removeLabel(repo, num, lbl);
  }

  // Step 2 (spec doc 3 §3.3): add the appropriate label for the new status.
  // For "new" no label is added — absence of github-scan:* labels IS "new".
  if (status === "wip" || status === "human" || status === "done") {
    const label = `github-scan:${status}` as const;
    const meta = GITHUB_SCAN_LABEL_META[label];
...
async function cmdList(
  ...
  for (const entry of inbox.notifications) {
    if (entry.github_scan_status === filterStatus) {
      io.stdout(entry.id);
    }
  }
  return 0;
...
async function cmdEnsureLabels(
  ...
  const gh = deps.gh ?? new GhClient();
  io.stdout(`Creating github-scan labels on ${repo}...`);
  for (const label of ALL_GITHUB_SCAN_LABELS) {
    const meta = GITHUB_SCAN_LABEL_META[label];
