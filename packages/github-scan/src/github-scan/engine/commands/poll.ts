 ...
 * GraphQL query, classifies into `github_scan_status`, then writes `inbox.json`
 ...
 *   - `runtime/paths.ts`     : `$GITHUB_SCAN_DIR` layout
 *
 * Spec references:
...
export interface PollDeps {
  ...
  /** `claimTimeoutSecs` override; default from loadGitHubScanConfig. */
...
/** Derive `github_scan_status` for every entry in place (classifier is pure). */
export function classifyEntries(entries: InboxEntry[]): void {
  for (const entry of entries) {
    entry.github_scan_status = classifyGitHubScanStatus({
      labels: entry.labels,
...
interface DiffEvent {
  kind: "new" | "transition";
  entry: InboxEntry;
  from?: InboxEntry["github_scan_status"];
  to?: InboxEntry["github_scan_status"];
}
...
export function diffEvents(
  ...
  if (old) {
    for (const entry of old.notifications) {
      prevStatuses.set(entry.id, entry.github_scan_status);
    }
  }
  ...
  for (const entry of next) {
    ...
    if (prev === entry.github_scan_status) continue;
    if (prev === "new" && entry.github_scan_status === "done") continue;
    events.push({
      kind: "transition",
      entry,
      from: prev,
      to: entry.github_scan_status,
    });
  }
  return events;
}
...
export async function runPoll(
  ...
  if (argv[0] === "--help" || argv[0] === "-h" || argv[0] === "help") {
    ...
    io.stdout("");
    io.stdout("Honors $GITHUB_SCAN_DIR (default ~/.first-tree/github-scan).");
    return 0;
  }

  const io = deps.io ?? DEFAULT_IO;
  const config = loadGitHubScanConfig();
  ...
  const newCount = entries.filter((e) => e.github_scan_status === "new").length;
  const timeOnly = pollTs.slice(11, 19);
  io.stdout(`github-scan: polled ${timeOnly} — ${total} notifications (${newCount} new)`);
  return 0;
}
