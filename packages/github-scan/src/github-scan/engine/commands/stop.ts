...
export async function runStop(
  ...
  try {
    ...
    write(
      `github-scan: could not resolve identity to stop the runner: ${err instanceof Error ? err.message : String(err)}`,
    );
    return 1;
  }
  ...
  if (!lock) {
    write("github-scan: no running github-scan-runner for the active identity");
    return 0;
  }
  ...
  if (isLockStale(lock)) {
    ...
    write(`removed stale github-scan-runner lock for pid ${lock.pid}`);
    return 0;
  }

  const code = killProcess(lock);
  if (code !== 0) {
    write(`github-scan: kill ${lock.pid} failed (exit ${code})`);
    return 1;
  }
  const stopped = await waitForStop(lock.pid);
  if (!stopped) {
    write(`github-scan: pid ${lock.pid} did not exit after SIGTERM; forcing stop`);
    const killCode = killProcess(lock, "-KILL");
    if (killCode !== 0) {
      write(`github-scan: kill -KILL ${lock.pid} failed (exit ${killCode})`);
      return 1;
    }
    if (!(await waitForStop(lock.pid, 1_000))) {
      write(`github-scan: pid ${lock.pid} is still alive after SIGKILL`);
      return 1;
    }
  }
  const dir = serviceLockDir(locksDir, identity, profile);
  ...
  write(`stopped github-scan-runner pid ${lock.pid}`);
  return 0;
}
