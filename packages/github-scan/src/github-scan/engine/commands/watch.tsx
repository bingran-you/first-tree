/**
 * TS port of `github-scan-watch` + `WATCH_DESIGN.md`.
 *
 * A read-only TUI board rendered with `ink` (React-for-terminals):
 *   - header: "github-scan status board" + status-count summary
 ...
 *
 * Read-only: watches `~/.first-tree/github-scan/inbox.json` (polled) and
 * `~/.first-tree/github-scan/activity.log` (tailed via fs.watch). No writes.
 *
...
function statusColor(s: GitHubScanStatus): string {
  switch (s) {
    case "human":
      return COLOR.red;
    case "new":
...
function statusLabel(s: GitHubScanStatus): string {
...
const HeaderBanner = (): React.ReactElement => {
  // Rainbow-tinted "github-scan" letters; each letter a fixed colour.
...
const Board = ({ inbox }: BoardProps): React.ReactElement | null => {
  const groups = useMemo(() => {
    ...
    for (const n of inbox.notifications) {
      if (n.github_scan_status === "human") continue;
      const row = byRepo.get(n.repo) ?? { new: [], wip: [], done: [] };
      if (n.github_scan_status === "new") row.new.push(n);
      else if (n.github_scan_status === "wip") row.wip.push(n);
      else if (n.github_scan_status === "done") row.done.push(n);
...
export interface GitHubScanWatchProps {
  inbox: Inbox | null;
  events: readonly ActivityEvent[];
}
 ...
 * `GitHubScanWatch` (the pure view) so ink-testing-library can render the view
...
const WatchApp = ({
  ...
  return <GitHubScanWatch inbox={inbox} events={events} />;
};

/**
 * Entry point for `github-scan watch`.
 *
