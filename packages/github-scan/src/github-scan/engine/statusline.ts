 ...
 *
 * This file is its own dist bundle (`dist/github-scan-statusline.js`). It is
 ...
 *
 * Behaviour is a line-for-line port of `github-scan-status`
 * +  `github-scan-statusline-wrapper` (the wrapper just passes stdin through and
 ...
 *   `/github-scan: ⚠ 2 need-you · 52 PRs · 3 issues (+1 new)`
...
/** 10 minutes; matches `bin/github-scan-status:25`. */
const STALE_THRESHOLD_SECS = 600;

function resolveGitHubScanDir(): string {
  const override = process.env.GITHUB_SCAN_DIR;
  if (override && override.length > 0) return override;
  return join(homedir(), ".first-tree/github-scan");
}

/**
 * Parse the `github_scan_status` counts from an inbox.json payload WITHOUT
...
function readCounts(inboxPath: string): Counts | null {
  ...
  let parsed: {
    last_poll?: string;
    notifications?: Array<{ github_scan_status?: string; type?: string }>;
  };
  try {
...
export function main(
  ...
  const githubScanDir = resolveGitHubScanDir();
  const inboxPath = join(githubScanDir, "inbox.json");
  const bellStatePath = join(githubScanDir, ".bell_state");
  ...
  try {
    ...
    if (nowSecs - mtimeSecs > STALE_THRESHOLD_SECS) {
      process.stdout.write("/github-scan: stale (poller not running?)\n");
      return 0;
    }
  } catch {
...
// Run only when this file is executed directly (i.e. `node dist/github-scan-statusline.js`),
// not when imported.
const isDirectRun =
  typeof process !== "undefined" &&
  Array.isArray(process.argv) &&
  process.argv[1] !== undefined &&
  /github-scan-statusline(\.[cm]?js)?$/u.test(process.argv[1]);
if (isDirectRun) {
  process.exit(main());
}
