...
function mkGitHubScanDir(): { dir: string; inbox: string; activity: string; claims: string } {
  const dir = mkdtempSync(join(tmpdir(), "github-scan-sm-"));
...
describe("status-manager: get", () => {
  let ctx: ReturnType<typeof mkGitHubScanDir>;
  beforeEach(() => {
    ctx = mkGitHubScanDir();
  });
  afterEach(() => rmSync(ctx.dir, { recursive: true, force: true }));

  it("returns github_scan_status for an existing id", async () => {
...
describe("status-manager: set", () => {
  ...
  it("removes all github-scan labels then adds the target label and patches inbox", async () => {
    const calls: string[][] = [];
    ...
    expect(addCalls[0][addCalls[0].indexOf("--add-label") + 1]).toBe("github-scan:wip");

    // Inbox was patched optimistically.
    const reparsed = JSON.parse(readFileSync(ctx.inbox, "utf-8")) as {
      notifications: Array<{ id: string; github_scan_status: string }>;
    };
    const entry = reparsed.notifications.find((n) => n.id === "23576674030");
    expect(entry?.github_scan_status).toBe("wip");
...
describe("status-manager: ensure-labels", () => {
  ...
  it("creates all four github-scan labels with correct colors", async () => {
    const calls: string[][] = [];
    const gh = new GhClient({
      spawn: vi.fn().mockImplementation((_cmd, argv: string[]) => {
        calls.push([...argv]);
    ...
    expect(code).toBe(0);
    expect(stdout.join("\n")).toContain("Creating github-scan labels on owner/repo");
    ...
    expect(colorFor("github-scan:human")).toBe("d93f0b");
    expect(colorFor("github-scan:done")).toBe("0e8a16");
  });
});
...
describe("status-manager: help", () => {
  it("prints usage and exits 0", async () => {
    ...
    expect(code).toBe(0);
    expect(stdout[0]).toMatch(/Usage: github-scan-status-manager/u);
  });
});
