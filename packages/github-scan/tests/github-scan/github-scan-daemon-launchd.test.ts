...
describe("renderLaunchdPlist", () => {
  ...
  it("produces a well-formed plist with KeepAlive and RunAtLoad", () => {
    ...
    expect(xml).toContain("<string>com.first-tree.github-scan.runner.alice.default</string>");
...
describe("start command self-invocation helpers", () => {
  it("resolves the current node + cli entrypoint pair", () => {
    const self = resolveSelfCliInvocation("/opt/github-scan/dist/cli.mjs");
    expect(self.executable).toBe(process.execPath);
    expect(self.prefixArgs).toEqual(["/opt/github-scan/dist/cli.mjs"]);
  });
