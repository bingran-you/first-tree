...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `github-scan-dispatch-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("Dispatcher execution", () => {
  ...
  it("hydrates snapshots before agent execution and passes tree repo through", async () => {
    ...
    env.dispatcher.submit(fakeCandidate());
    await waitForCompletions(env, 1);
    expect(snapshotWriter.writeTaskSnapshot).toHaveBeenCalledTimes(1);
    expect(env.completions[0].summary).toBe("snapshot ok");
  });
