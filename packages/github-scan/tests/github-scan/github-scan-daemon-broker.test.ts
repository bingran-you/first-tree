...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `github-scan-broker-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("SHIM_SCRIPT", () => {
  it("references GITHUB_SCAN_BROKER_DIR and writes argv.txt", () => {
    expect(SHIM_SCRIPT).toContain("GITHUB_SCAN_BROKER_DIR");
